...
new_fn_sql = r"""
drop function is_similar(needle text, haystack text, max_distance real);

create or replace function is_similar(needle text, haystack text, algorithm text default 'trigram', distance real default 0.5) returns boolean as $$
declare
  needle_tokens text[];
  haystack_tokens text[];
  found_tokens text[];
  ...
  foreach needle_token in array needle_tokens loop
    ...
    foreach haystack_token in array haystack_tokens loop
      ...
      if algorithm = 'levenshtein' then
        needle_found := levenshtein(needle_token, haystack_token) <= distance;
      elsif algorithm = 'trigram' then
        needle_found := char_length(needle_token) > 1 and char_length(haystack_token) > 1 and similarity(needle_token, haystack_token) >= (1-distance);
      end if;
