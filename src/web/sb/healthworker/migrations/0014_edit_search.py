...
create_fn_sql = r"""
create or replace function edit_search(needle text, haystack text, max_edits integer) returns boolean as $$
declare
  needle_tokens text[];
...
begin
  ...
  foreach needle_token in array needle_tokens loop
    ...
    foreach haystack_token in array haystack_tokens loop
      ...
      needle_found = levenshtein(needle_token, haystack_token) <= max_edits;
    end loop;
