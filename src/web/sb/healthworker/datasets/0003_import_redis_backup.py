...
def import_user_progress(user):
  ...
  status = _first(RegistrationStatus.objects.filter(msisdn=msisdn))
  if status is None:
  ...
  for k in user.get('answers', {}).keys():
    ...
    answer = _first(RegistrationAnswer.objects.filter(msisdn=msisdn, question=state))
    if answer is None:
...
def run():
  path = _helpers.get_path('kv-backup-20130715.json')
